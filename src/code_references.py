...
import re
CODE_REGEX = {
    "CCIV": r"(?P<CCIV>(?i:Code\scivil|C\.?\s?civ\.?|Code\sciv\.|civ\.)|CCIV)",
    "CPRCIV": r"(?P<CPRCIV>(?i:Code\sde\sprocédure\scivile|C\.?\s?pr\.\s?civ\.?)|CPC)",
    "CCOM": r"(?P<CCOM>(?i:Code\sd(e|u)\scommerce|C\.?\s?com\.?))",
    "CTRAV": r"(?P<CTRAV>(?i:Code\sdu\stravail|C\.?\s?trav\.?))",
    "CPI": r"(?P<CPI>(?i:Code\sde\sla\spropriété\sintellectuelle|C\.?\s?pr\.\s?int\.)|CPI)",
    "CPEN": r"(?P<CPEN>(?i:Code\sp(é|e)nal|C\.?\s?p(é|e)n\.?))",
    "CPP": r"(?P<CPP>(?i:Code\sde\sprocédure\sp(é|e)nale)|CPP)",
    "CASSUR": r"(?P<CASSUR>(?i:Code\sdes\sassurances|C\.?\s?assur\.?))",
    "CCONSO": r"(?P<CCONSO>(?i:Code\sde\sla\sconsommation|C\.?\s?conso\.?))",
    "CSI": r"(?P<CSI>Code\sde\slas(e|é)curité\sint(é|e)rieure|CSI)",
    "CSP": r"(?P<CSP>(?i:Code\sde\slasanté\spublique|C\.\ssant\.\spub\.)|CSP)",
    "CSS": r"(?P<CSS>(?i:Code\sde\sla\ss(é|e)curit(é|e)\ssociale|C\.\ssec\.\ssoc\.)|CSS)",
    "CESEDA": r"(?P<CESEDA>(?i:Code\sde\sl'entr(é|e)e\set\sdu\ss(é|e)jour\sdes\s(é|e)trangers\set\sdu\sdroit\sd'asile)|CESEDA)",
    "CGCT": r"(?P<CGCT>(?i:Code\sgénéral\sdes\scollectivit(e|é)s\sterritoriales)|CGCT)",
    "CPCE": r"(?P<CPCE>(?i:Code\sdes\spostes\set\sdes\scommunications\s(é|e)lectroniques)|CPCE)",
    "CENV": r"(?P<CENV>(?i:Code\sde\sl'environnement|C.?\s?envir.|C\.?\s?Env)|C\.E|CENV)",
    "CJA": r"(?P<CJA>(?i:Code\sde\sjustice\sadministrative)|C\.?\s?JA)",
    "CTRANS": r"(?P<CTRANS>(?i:Code\sdes\stransports)|C\.?\s?TRANS)",
    "CMF": r"(?P<CMF>(?i:Code\smon(é|e)taire\set\sfinancier)|C\.?\s?MF)"
}
